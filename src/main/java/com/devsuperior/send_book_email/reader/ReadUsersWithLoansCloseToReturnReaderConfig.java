package com.devsuperior.send_book_email.reader;
...
import com.devsuperior.send_book_email.domain.Book;
import com.devsuperior.send_book_email.domain.User;
...
public class ReadUsersWithLoansCloseToReturnReaderConfig {
	...
	public ItemReader<UserBookLoan> readUsersWithLoansCloseToReturnReader(@Qualifier("appDS") DataSource dataSource) {
		return new JdbcCursorItemReaderBuilder<UserBookLoan>().name("readUsersWithLoansCloseToReturnReader")
				...
				.sql("SELECT " + "user.id as user_id," + "user.name as user_name," + "user.email as user_email,"
						+ "book.id as book_id," + "book.name as book_name," + "loan.loan_date "
